...
public class PathCursor extends AbstractCursor {
    ...
    public PathCursor(File parentDirectory, File[] fileList) {
    ...
    public static Comparator<FileItem> sComparator = new Comparator<FileItem>() {
        @Override
        public int compare(FileItem ls, FileItem rs) {
            if (ls.isDirectory && !rs.isDirectory)
                return -1;
            else if (!ls.isDirectory && rs.isDirectory)
                return 1;
            return ls.file.getName().compareToIgnoreCase(rs.file.getName());
        }
    };
